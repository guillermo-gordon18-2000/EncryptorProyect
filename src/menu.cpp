...
void displayMenu(WINDOW *menu_win, const vector<string>& recentEncryptedFiles, const vector<string>& recentDecryptedFiles) {
    ...
    mvwprintw(menu_win, 4, 2, "  2. Cifrar clave");
    mvwprintw(menu_win, 5, 2, "  3. Cifrar enlaces");
    mvwprintw(menu_win, 6, 2, "  4. Salir");
    ...
    wattron(menu_win, COLOR_PAIR(5)); // Color amarillo
    mvwprintw(menu_win, 21, 2, "Seleccione una opción:");
    wattroff(menu_win, COLOR_PAIR(5));
...
void displayEncrypFile(WINDOW *menu_win, const vector<string>& recentEncryptedFiles, const vector<string>& recentDecryptedFiles) {
    werase(menu_win);  // Limpiar toda la ventana
    box(menu_win, 0, 0);  // Volver a dibujar el borde
